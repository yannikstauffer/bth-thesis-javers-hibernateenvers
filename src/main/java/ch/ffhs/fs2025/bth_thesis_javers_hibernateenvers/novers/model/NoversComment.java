package ch.ffhs.fs2025.bth_thesis_javers_hibernateenvers.novers.model;

import ch.ffhs.fs2025.bth_thesis_javers_hibernateenvers.common.BaseEntity;
import ch.ffhs.fs2025.bth_thesis_javers_hibernateenvers.common.Comment;
import jakarta.persistence.CascadeType;
import jakarta.persistence.Entity;
import jakarta.persistence.JoinColumn;
import jakarta.persistence.ManyToOne;
import jakarta.persistence.OneToMany;
import lombok.AllArgsConstructor;
import lombok.Getter;
import lombok.NoArgsConstructor;
import lombok.Setter;

import java.util.ArrayList;
import java.util.List;

@Getter
@Setter
@NoArgsConstructor
@AllArgsConstructor
@Entity(name = "novers_comments")
public class NoversComment extends BaseEntity implements Comment<NoversComment, NoversThread, NoversUser> {

    protected String content;

    @ManyToOne
    @JoinColumn(name = "parent_id")
    private NoversComment parent;

    @OneToMany(mappedBy = "parent", cascade = CascadeType.ALL, orphanRemoval = true)
    private List<NoversComment> children = new ArrayList<>();

    @ManyToOne
    @JoinColumn(name = "owner_id")
    private NoversUser owner;

    @ManyToOne
    @JoinColumn(name = "thread_id")
    private NoversThread thread;


}